...
void IsSave( student_ *pstu )  //已修改
{
    ...
    {
        if (ShowIsSave())
            DoSave(pstu,DataNum);//执行前面主程序里面的保存数据
    }
}
...
int OpenData( student_ **pstu, int num )//打开文件，建立链表，将指定班级下的第一个学生信息的首地址赋给pstu   已修改
{
    ...
    while( pend && pend-pos>=rc )
    {
        ...
        fread( &pnew->stu, sizeof(STUDENT), 1, fp );//从fp所指向的班级文件中读取1个STUDENT型数据到pnew，即完成对当前pnew的赋值
        pnew->next=NULL;
...
int SaveData( student_ *pstu, int num )//保存班级文件信息    已更新
{
    ...
    {
        if ( pnew )
            fwrite( &pnew->stu, sizeof(STUDENT), 1, fp );//将pnew(当下pnew所指向的学生的信息)的地址写到fp指向的文件中
...
int SearchData(student_ *pstu, int num, int item, char *name, float score, int dd )//根据不同的数据找学生信息（一次只能根据一个标准） 已修改
{
    ...
    for (i=0; i<num; i++ )//对该班级下所有的学生信息(各个项)扫描一遍
    {
        ...
        pnew=pnew->next;//不论是否满足条件，指针都要往后移动一个单位
    }
    printf( "___________________________________________________________________\n" );
    if ( flag )
        return TRUE;//flag=1表示在当下班级里面找到了符合条件的学生
    else
        return FALSE;
}
...
void InputOneData( student_ *pstu, int i )//为修改第i个学生的信息而重新输入一组数据,直接通过指针修改 已修改
{
    student_ *pnew=pstu;
    char sex;
    ...
    pnew->stu.sex = (sex=='M') ? man : female;
    printf("年龄: ");
    scanf("%d", &pnew->stu.age);
    fflush(stdin);
    printf("班级: ");
    scanf("%d", &pnew->stu.classno);
    fflush(stdin);
    printf("数学: ");
    scanf("%f", &pnew->stu.Math);
    fflush(stdin);
    printf("英语: ");
    scanf("%f", &pnew->stu.English);
    fflush(stdin);
    printf("C语言: ");
    scanf("%f", &pnew->stu.C_Language);
    fflush(stdin);//直接通过指针修改学生信息
}
...
{                                                  //(优化) 如果是成绩，则从大到小排序；如果是学号、姓名则从小到大排序
    student_ *pnew=pstu;
    student_ *point_=pnew;

    while(pnew)//选择排序
    {
        student_ *q=pnew;//这里要注意更新q(找下一个值的起始位置)
        point_=pnew;
        while(q) //在排序时根据不同标准来排序
        {
            ...
            {
            case 1:
                if ( strcmp(q->stu.no, point_->stu.no)<0 )
                    point_=q;
                break;
            case 2:
                if ( strcmp(q->stu.name, point_->stu.name)<0 )
                    point_=q;
                break;
            case 3:
                if ( q->stu.English > point_->stu.English )
                    point_=q;
                break;
            case 4:
                if ( q->stu.Math > point_->stu.Math )
                    point_=q;
                break;
            case 5:
                if ( q->stu.C_Language > point_->stu.C_Language )
                    point_=q;
                break;
            }
            q=q->next;
...
void tongjiall( student_ *pstu, int num )//做统计[可优化？？]  已修改
{
    ...
    for ( i=0; i<num; i++ )
    {
        if ( pnew )
        {
            ...
            if ( maxalle<pnew->stu.English)
                maxalle = pnew->stu.English;
            if ( minalle>pnew->stu.English)
                minalle = pnew->stu.English;
            avralle += pnew->stu.English;
            if ( maxallm<pnew->stu.Math)
                maxallm = pnew->stu.Math;
            if ( minallm>pnew->stu.Math)
                minallm = pnew->stu.Math;
            avrallm += pnew->stu.Math;
            if ( maxallc<pnew->stu.C_Language)
                maxallc = pnew->stu.C_Language;
            if ( minallc>pnew->stu.C_Language)
                minallc = pnew->stu.C_Language;
