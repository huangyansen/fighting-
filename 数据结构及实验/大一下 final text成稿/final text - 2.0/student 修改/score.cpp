...
void ShowQuit( STUDENT *pstu )  //已修改
{
	IsSave(pstu);
	printf( "\n谢谢使用本软件!\n" );
	printf( "欢迎下次再使用！\n" );
...
int OpenData( STUDENT **pstu, int num )//打开文件，建立链表，将指定班级下的第一个学生信息的首地址赋给pstu   已修改
{
	...
	while( pend && pend-pos>=rc )
	{
		...
		//！！这里记得要减去4个字节，因为老师给的文件里面结构体中没有指针(4字节)
		pnew->next=NULL;
...
int SaveData( STUDENT *pstu, int num )//保存班级文件信息    已更新
{
	int i;
	FILE *fp;
	...
	{
		if ( pnew )
			fwrite( pnew, sizeof(STUDENT), 1, fp );//将pnew(当下pnew所指向的学生)的地址写到fp指向的文件中
...
STUDENT *InputItem(void)//在已有学生后面追加登录信息，返回存有完整信息的地址  已修改
{
    STUDENT *p;
    char sex;
...
void ShowOneItem( STUDENT *pstu, int i )//输出当前班级下第i个学生的相关信息  已修改
{
    STUDENT *p;
    p=pstu;
    for(int j=1;j<i;++j)
...
int SearchData(STUDENT *pstu, int num, int item, char *name, float score, int dd )//根据不同的数据找学生信息（一次只能根据一个标准） 已修改
{
	...
	for (i=0; i<num; i++ )//对该班级下所有的学生信息(各个项)扫描一遍
	{
		...
		if ( dd==1 && !strcmp(pnew->no,name)	||
			 dd==2 && !strcmp(pnew->name,name) ||
			 dd==3 && pnew->sex==item			||
			 dd==4 && pnew->age==item			||
			 dd==5 && pnew->classno==item		||
			 dd==6 && pnew->English==score		||
			 dd==7 && pnew->Math==score		||
			 dd==8 && pnew->C_Language==score		)//对于第i个学生，在各类判断标准下，找到与输入信息条件相等的
		{
			ii = i;//第i个学生符合
...
int SearchNo(STUDENT *pstu, int num, char *name )//num是总人数，根据学号name来找学生  已修改
{
	int i;
	STUDENT *pnew;
	pnew=pstu;
	...
	{
		if (!pnew) { pnew=pnew->next;continue;}
		if (!strcmp(pnew->no,name) ) return i;//找到则返回是第i个人
		pnew=pnew->next;//既非空，又匹配不上学号，就往下找一个
	}
	return -1;//没找到返回-1
}

void InputOneData( STUDENT *pstu, int i )//为修改第i个学生的信息而重新输入一组数据 【可以优化，想修改什么。。数据】  已修改
{
    STUDENT *p, *pnew=pstu;
    char sex;
    ...
    strcpy(p->no, pnew->no);//再将原学生的学号赋给p
    pnew = p;//将新的完整的学生信息重新赋给pnew, 就相当于将新的信息地址写入第i个学生的地址
    free(p);//释放申请的空间
}


void Swap(STUDENT *p, STUDENT *q)  //已修改
{
     STUDENT temp = *p;
     temp.next = q->next;
     q->next = p->next;
...
void SortStuInfo( STUDENT *pstu, int num, int dd )//根据不同标准对学生信息进行排序，num表示该标班级的总人数  ,由大到小排序  已修改
{
    STUDENT *p,*pnew=pstu->next;
    STUDENT *max_=pnew;
    int i, j, k;
	...
	while(pnew)//选择排序
	{
		...
		while(q){//在排序时根据不同标准来排序
			switch( dd )
			{
				case 1: if ( strcmp(q->no, max_->no)>0 )	max_=q;	break;
				case 2: if ( strcmp(q->name, max_->name)>0 )	max_=q;	break;
				case 3: if ( q->English > max_->English )	max_=q;	break;
				case 4: if ( q->Math > max_->Math )		max_=q;	break;
				case 5: if ( q->C_Language > max_->C_Language )	max_=q;	break;
			}
...
void tongjiall( STUDENT *pstu, int num )//做统计[可优化？？]  已修改
{
	int i,cc=0;
	STUDENT *pnew;
	pnew=pstu;
	...
	for ( i=0; i<num; i++ )
	{
		if ( pnew )
		{
			avr = (pnew->English+pnew->Math+pnew->C_Language)/(float)3.0;//第i个学生三门课的平均分
			max_ = pnew->English>pnew->Math ? pnew->English : pnew->Math;
			max_ = max_>pnew->C_Language ? max_ : pnew->C_Language;
			min_ = pnew->English<pnew->Math ? pnew->English : pnew->Math;
			min_ = min_<pnew->C_Language ? min_ : pnew->C_Language;//第i个学生三门课的最高和最低分
			if ( maxalle<pnew->English) maxalle = pnew->English;
			if ( minalle>pnew->English) minalle = pnew->English;
			avralle += pnew->English;
			if ( maxallm<pnew->Math) maxallm = pnew->Math;
			if ( minallm>pnew->Math) minallm = pnew->Math;
			avrallm += pnew->Math;
			if ( maxallc<pnew->C_Language) maxallc = pnew->C_Language;
			if ( minallc>pnew->C_Language) minallc = pnew->C_Language;
			avrallc += pnew->C_Language;//分别找三门课中的最高，最低分，以及计算所以学生各门课的成绩和
			printf( "%10s%12s%8.1f%8.1f%8.1f\n", pnew->no, pnew->name, max_, min_, avr );//第i个学生的统计信息
			cc++;//统计总人数
		}
    pnew=pnew->next;
	}
...
void FreeMemory(STUDENT *pstu)//销毁链表
{
   STUDENT *p,*q;
