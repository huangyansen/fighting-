...
void DoMainMenu( STUDENT *pstu )//主菜单操作
{
	...
	while ( 1 )
	{
		...
		if ( dd==0 )
		{
			ShowQuit(pstu);//退出该程序，如果有修改过，就提示是否要保存修改数据         ！！！！！！！！！！！！！！！！！此处未修改
			break;
		}
		else if ( dd<0 || dd>=9 )
		{
...
void Doinput( STUDENT *pstu, int num )//登录数据，即在已打开的班级后面追加学生信息
{
    	...
    	for ( i=0; i<cc; i++ )
    	{
        	tail->next = InputItem();//在已有学生后面追加登录信息
        	if (tail->next==NULL)
        	{
            		printf("\n内存分配有错!\n");
            		FreeMemory (pstu);//未分配到空间，销毁链表
            		exit(-1);
       		}
       		tail=tail->next;
...
void DoShow( STUDENT *pstu, int num )//显示数据 已修改
{

	STUDENT *p;
	p=pstu;
	//为什么这里pstu指向内容的地址(即pstu的值比放入进去的小了208)?
