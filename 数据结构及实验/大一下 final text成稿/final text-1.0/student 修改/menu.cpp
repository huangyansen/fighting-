...
void DoMainMenu( STUDENT **pstu )//主菜单操作
{
	...
	char showitem[9][31]={	"1   打开文件",
				...
				"8   统计数据",
				"0   退出系统" };
	void (*func[])(STUDENT **, int) = { DoOpen, DoSave, Doinput, DoSearch, DoEdit, DoShow, DoSort, Dotongji };//把函数名赋值给func函数指针
	...
	while ( 1 )
	{
		...
		else func[dd-1](pstu,DataNum);//输入正确，执行输入对应的操作，通过函数指针来实现（相当于执行上述的9个不同程序）
	}
}

//分别解释每个操作
void DoOpen( STUDENT **pstu, int num )//打开文件(必须要先打开文件）作用：  选择一个班级进行后续操作：从2~8
{
	...
	while ( 1 )
	{
		...
		else
		{
			...
			if ( !OpenData( pstu, num ) )//打开班级文件，执行之后就把 func[dd-1](pstu,DataNum)中的DataNum改为该班级总人数了
			{
...
void DoSave( STUDENT **pstu, int num )//在对班级下面学生的信息进行修改后，要对班级文件信息进行保存
{
	char TitleName[30] = "请选择一个文件";
	char showitem[8][31] = {	"1  01班学习成绩文件",
					"2  02班学习成绩文件",
	...
	int  dd;
	if (!strcmp(WorkingFileName,""))//若WorkingFileName为空，则执行下面程序选择班级来保存
	{
		while ( 1 )
		{
			ShowMenu( TitleName, showitem, 8 );
			...
			{
				strcpy( WorkingFileName, FileName[dd-1] );//赋值班级名
				break;
			}
		}
	}
...
void Doinput( STUDENT **pstu, int num )//登录数据，即在已打开的班级后面追加学生信息
{
    	int i,cc;
    	printf( "\n\n数据登录操作！\n" );
    	while(1)
...
void DoSearch( STUDENT **pstu, int num )//查找数据
{
		...
		else
		{
			...
			else if ( dd==4 )
			{
				while(1)
				{
					...
					scanf("%d", &item);
					fflush(stdin);
					if ( item>15 && item<32 ) break;//年龄只能在15到32之间
					else ShowMessage(  "","数据输入有错，请重新输入！" );
				}
			}
			else if ( dd==5 )
			...
			if ( !SearchData(pstu,num,item,name,score,dd) )
                //在该班级文件下查找并显示[一次只有一个数据有效]num(该班级下学生总数) item(性别，年龄，班级) name(学号，姓名) score(三科成绩) dd(数据项）
...
void DoEdit( STUDENT **pstu, int num )//修改数据
{
	int  dd;
	char no[31];
...
void DoShow( STUDENT **pstu, int num )//显示数据
{
	int i;
    	printf ("\n==========================班级数据表===========================\n");
    	for (i=0; i<num; i++)
    	{
		if(pstu[i])//若不是空指针，就显示出该班级下所有学生的信息
...
void DoSort( STUDENT **pstu, int num )//根据不同的标准进行排序
{
	...
	while ( 1 )
	{
		...
		scanf( "%d", &dd );
		if ( dd==0 ) break;//返回上一级
		if ( dd<0 || dd>=6 ) ShowSelectError(6);//输入错误，重新选择
		else
		{
...
void Dotongji( STUDENT **pstu, int num )//统计数据
{
	tongjiall( pstu, num );
	ShowMessage( WorkingFileName, "\n统计结束！");
}
